...
WITH Student (student_id, student_name)
AS 
(
	SELECT 1,'Daniel  ' UNION ALL
...
FROM(
	SELECT *,
			MAX(Exam.score) OVER(PARTITION BY Exam.exam_id) maxScore,
			MIN(Exam.score) OVER(PARTITION BY Exam.exam_id) minScore
	FROM Exam 
) S 
WHERE S.score = S.maxScore OR s.score = S.minScore
)

SELECT DISTINCT Exam.student_id, Student.student_name 
FROM Exam 
JOIN Student ON Student.student_id = Exam.student_id
WHERE Student.student_id != all( 
	SELECT solution.student_id FROM solution
)